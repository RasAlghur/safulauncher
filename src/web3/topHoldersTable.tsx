...
export function TopHoldersTable({ tokenAddress, creatorAdress, bondingAddr }: TopHoldersTableProps) {
  ...
  async function fetchTopHolders() {
    ...
    try {
      ...
      const response = await Moralis.EvmApi.token.getTokenOwners({
        ...
        order: "DESC",
        tokenAddress,               // pass the string directly
        // page: 1,
        // pageSize: 100,
      });
      ...
      // Map Moralis result into { owner, percent } shape:
  ...
  if (loading) return <p className="text-sm">Loading top holders…</p>;
        ...
        <table className="min-w-full text-sm dark:text-white/80">
          <thead>
            <tr>
              ...
              <th className="py-2 px-2 text-left">% of Supply</th>
            </tr>
          </thead>
          <tbody>
