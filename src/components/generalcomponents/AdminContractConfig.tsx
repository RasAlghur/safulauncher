...
const AdminContractConfig = () => {
    ...
    } = useBalance({ address: SAFU_LAUNCHER_CA_V2 });
    ...
    const validatePercentage = (value: string, max: number = 100): string | null => {
        const num = parseFloat(value);
        if (isNaN(num)) return "Must be a valid number";
        if (num < 0) return "Cannot be negative";
    ...
    const validateAddress = (value: string): string | null => {
        ...
        if (!/^0x[a-fA-F0-9]{40}$/.test(value)) return "Invalid Ethereum address format";
        return null;
    };

    const validateTierHierarchy = (): string | null => {
        if (!tier1Threshold || !tier1ThresholdDiv || !tier2Threshold || !tier2ThresholdDiv) {
            return null;
        }

        const tier1Percent = (parseFloat(tier1Threshold) / parseFloat(tier1ThresholdDiv)) * 100;
        const tier2Percent = (parseFloat(tier2Threshold) / parseFloat(tier2ThresholdDiv)) * 100;
    ...
    const handleValidation = (fieldName: string, value: string, validator: (val: string) => string | null) => {
    ...
    const FormSection = ({ title, children, tooltip }: { title: string; children: React.ReactNode; tooltip?: string }) => (
                    ...
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
                        ...
                        {currentValue !== undefined && (
                            <span className="ml-2 text-xs text-gray-500 dark:text-gray-400">
                                (Current: {currentValue.toString()}{unit && ` ${unit}`})
                            </span>
                        )}
                    </label>
                    {tooltip && (
    ...
    const ActionButton = ({
        ...
        const baseClasses = "px-4 py-2 rounded-md font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2";
    ...
    const isSafuTokenValid: boolean =
        !validationErrors.safuTokenAddress &&
        safuTokenAddress.trim() !== "";
    ...
    return (
        <div className="p-4 max-w-6xl mx-auto mt-8 space-y-6">
            ...
            {/* Contract Metrics */}
            <FormSection title="📊 Contract Information" tooltip="Current contract state and metrics">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
                    <div className="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
                        <div className="text-2xl font-bold text-blue-600 dark:text-blue-400">
                            {(Number(currentTradeFeeBps) / 100) || "0"}%
                        </div>
                        <div className="text-sm text-gray-600 dark:text-gray-400">Trade Fee (%)</div>
                    </div>
                    <div className="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
                        <div className="text-2xl font-bold text-green-600 dark:text-green-400">
                            {currentInitialPoolEth ? ethers.formatEther(currentInitialPoolEth.toString()) : "0"}
                        </div>
                        <div className="text-sm text-gray-600 dark:text-gray-400">Initial Pool ETH</div>
                    </div>
                    <div className="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
                        <div className="text-2xl font-bold text-purple-600 dark:text-purple-400">
                            {currentDevRewardETH ? ethers.formatEther(currentDevRewardETH.toString()) : "0"}
                        </div>
                        <div className="text-sm text-gray-600 dark:text-gray-400">Dev Reward ETH</div>
                    </div>
                    <div className="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
                    ...
                    <div className="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
                        ...
                        <div className="text-sm text-gray-600 dark:text-gray-400">Max Tax On Safu</div>
                    </div>
                    <div className="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
                        <div className="text-2xl font-bold text-orange-600 dark:text-orange-400">
                            {Number(getTaxOnDexMaxBps) / 100}%
                        </div>
                        <div className="text-sm text-gray-600 dark:text-gray-400">Max Tax On Dex</div>
                    </div>
                    <div className="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
                        <div className="text-2xl font-bold text-orange-600 dark:text-orange-400">
                            {getListingMilestone}%
                        </div>
                        <div className="text-sm text-gray-600 dark:text-gray-400">Listing Milestone</div>
                    </div>
                    <div className="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
                        <div className="text-2xl font-bold text-orange-600 dark:text-orange-400">
                            {getBundleMaxAmount}%
                        </div>
                        <div className="text-sm text-gray-600 dark:text-gray-400">Max Dev Bundle Amount</div>
                    </div>
                    <div className="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
                        <div className="text-2xl font-bold text-orange-600 dark:text-orange-400">
                            {Number(getMaxWhitelistBps) / 100}%
                        </div>
                        <div className="text-sm text-gray-600 dark:text-gray-400">Max Whitelist Amount</div>
                    </div>
                </div>
            </FormSection>
            ...
            {hash && (
                <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
                    ...
                    {isConfirming && <p className="text-blue-600 dark:text-blue-300 mt-2">Waiting for confirmation...</p>}
                    {isConfirmed && <p className="text-green-600 dark:text-green-400 mt-2">✅ Transaction confirmed!</p>}
                </div>
            )}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                {/* Emergency Functions */}
                <FormSection title="🚨 Emergency Functions" tooltip="To remove extra ETH from the safu launcher contract">
                    <div className="space-y-4">
                        <div className="bg-yellow-50 dark:bg-yellow-900/20 p-4 rounded-lg border border-yellow-200 dark:border-yellow-800">
                            <p className="text-yellow-800 dark:text-yellow-200 text-sm mb-3">
                                Contract Balance: {contractBalance ? (contractBalance.formatted) : "0"} ETH
                            </p>
                            <p className="text-yellow-800 dark:text-yellow-200 text-sm mb-3">
                                Reserved ETH: {geReservedETH ? ethers.formatEther(geReservedETH.toString()) : "0"} ETH
                            </p>
                            <ActionButton
                ...
                {/* Fee Configuration */}
                <FormSection title="💰 Fee Configuration" tooltip="Configure platform trading and listing fees">
                    <div className="space-y-4">
                        <InputField
                            label="Trade Fee (%)"
                            value={tradeFeeBps}
                ...
                {/* Pool Configuration */}
                <FormSection title="🏊 Pool Configuration" tooltip="Configure initial liquidity pool settings">
                    <div className="space-y-4">
                        <InputField
                            ...
                            placeholder="1.5"
                            type="number"
                            currentValue={currentInitialPoolEth ? ethers.formatEther(currentInitialPoolEth.toString()) : "0"}
                        ...
                        <InputField
                            ...
                            placeholder="0.2"
                            type="number"
                            currentValue={currentDevRewardETH ? ethers.formatEther(currentDevRewardETH.toString()) : "0"}
                ...
                <FormSection title="⚙️ Creator Configuration" tooltip="Configure limits for token creators">
                    <div className="space-y-4">
                        <div className="grid grid-cols-2 gap-3">
                            <InputField
                ...
                <FormSection title="🪙 SAFU Token" tooltip="Configure the SAFU token contract address">
                    <div className="space-y-4">
                        <InputField
                ...
                <FormSection title="🎯 SAFU Token Holders Whitelist Tiers" tooltip="Configure whitelist tiers for SAFU token holders">
                    <div className="space-y-4">
                        {tierHierarchyError && (
                            ...
                            <div className="border border-gray-200 dark:border-gray-600 rounded-lg p-4">
                                ...
                                <div className="mt-2 p-2 bg-blue-50 dark:bg-blue-900/20 rounded text-sm">
                                    <p className="text-blue-700 dark:text-blue-300">
                                        Current: {tier1Threshold && tier1ThresholdDiv ?
                                            `${((parseInt(tier1Threshold) / parseInt(tier1ThresholdDiv)) * 100).toFixed(3)}%` :
                                            "N/A"} threshold, {tier1WLCap && tier1WLDiv ?
                                                `${((parseInt(tier1WLCap) / parseInt(tier1WLDiv)) * 100).toFixed(3)}%` :
                                                "N/A"} cap
                                    </p>
                                </div>
                            </div>
                            ...
                            <div className="border border-gray-200 dark:border-gray-600 rounded-lg p-4">
                                ...
                                <div className="mt-2 p-2 bg-gray-50 dark:bg-gray-700 rounded text-sm">
                                    <p className="text-gray-700 dark:text-gray-300">
                                        Current: {tier2Threshold && tier2ThresholdDiv ?
                                            `${((parseInt(tier2Threshold) / parseInt(tier2ThresholdDiv)) * 100).toFixed(3)}%` :
                                            "N/A"} threshold, {tier2WLCap && tier2WLDiv ?
                                                `${((parseInt(tier2WLCap) / parseInt(tier2WLDiv)) * 100).toFixed(3)}%` :
                                                "N/A"} cap
                                    </p>
                                </div>
                            </div>
                        ...
                        <div className="bg-gray-50 dark:bg-gray-800 p-4 rounded-lg">
                            <h5 className="font-medium text-gray-700 dark:text-gray-300 mb-2">Configuration Summary</h5>
                            <div className="text-sm text-gray-600 dark:text-gray-400 space-y-1">
                                <p>• Tier 1 users need ≥{tier1Threshold && tier1ThresholdDiv ?
                                    `${((parseInt(tier1Threshold) / parseInt(tier1ThresholdDiv)) * 100).toFixed(3)}%` :
                                    "0"} of SAFU supply</p>
                                <p>• Tier 2 users need ≥{tier2Threshold && tier2ThresholdDiv ?
                                    `${((parseInt(tier2Threshold) / parseInt(tier2ThresholdDiv)) * 100).toFixed(3)}%` :
                                    "0"} of SAFU supply</p>
            ...
            <div className="bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-6">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
                    <div className={`flex items-center gap-2 ${isTradeFeeBpsValid ? 'text-green-600 dark:text-green-400' : 'text-gray-500 dark:text-gray-400'}`}>
                        <div className={`w-2 h-2 rounded-full ${isTradeFeeBpsValid ? 'bg-green-500' : 'bg-gray-300'}`}></div>
                        Trade Fee Config
                    </div>
                    <div className={`flex items-center gap-2 ${isPoolConfigValid ? 'text-green-600 dark:text-green-400' : 'text-gray-500 dark:text-gray-400'}`}>
                        <div className={`w-2 h-2 rounded-full ${isPoolConfigValid ? 'bg-green-500' : 'bg-gray-300'}`}></div>
                        Pool Config
                    </div>
                    <div className={`flex items-center gap-2 ${isListingFeeValid ? 'text-green-600 dark:text-green-400' : 'text-gray-500 dark:text-gray-400'}`}>
                        <div className={`w-2 h-2 rounded-full ${isListingFeeValid ? 'bg-green-500' : 'bg-gray-300'}`}></div>
                        Listing Fee Config
                    </div>
                    <div className={`flex items-center gap-2 ${isCreatorConfigValid ? 'text-green-600 dark:text-green-400' : 'text-gray-500 dark:text-gray-400'}`}>
                        <div className={`w-2 h-2 rounded-full ${isCreatorConfigValid ? 'bg-green-500' : 'bg-gray-300'}`}></div>
                        Creator Config
                    </div>
                    <div className={`flex items-center gap-2 ${isSafuTokenValid ? 'text-green-600 dark:text-green-400' : 'text-gray-500 dark:text-gray-400'}`}>
                        <div className={`w-2 h-2 rounded-full ${isSafuTokenValid ? 'bg-green-500' : 'bg-gray-300'}`}></div>
                        SAFU Token Config
                    </div>
                    <div className={`flex items-center gap-2 ${isTierConfigValid ? 'text-green-600 dark:text-green-400' : 'text-gray-500 dark:text-gray-400'}`}>
                        <div className={`w-2 h-2 rounded-full ${isTierConfigValid ? 'bg-green-500' : 'bg-gray-300'}`}></div>
                        Tier Config
                    </div>
                </div>
            </div>
