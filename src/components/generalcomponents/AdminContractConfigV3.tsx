...
import { LAUNCHER_ABI_V3, SAFU_LAUNCHER_ADDRESSES_V3 } from "../../web3/config";
...
const AdminContractConfigV3 = () => {
  ...
  // Add new state for lock duration and creation fee
  const [tLockDur, setTLockDur] = useState<string>("");
  const [lpLockDur, setLpLockDur] = useState<string>("");
  const [tLckPrcnt, setTLckPrcnt] = useState<string>("");
  const [creationFeeETH, setCreationFeeETH] = useState<string>("");
  const [creationHolderThresholdBps, setCreationHolderThresholdBps] =
    useState<string>("");
  ...
  const { data: getMetrics } = useReadContract({
  ...
  useEffect(() => {
    ...
    if (getMetrics === undefined) {
      setCombinedMetrics(null);
      return;
    }

    const metricsArray: bigint[] = Array.from(getMetrics);

    setCombinedMetrics(metricsArray);
  }, [getMetrics]);
  ...
  const validateLockPercentage = (value: string): string | null => {
    const num = parseFloat(value);
    if (isNaN(num)) return "Must be a valid number";
  ...
  const validateCreationFeeETH = (value: string): string | null => {
    const num = parseFloat(value);
    if (isNaN(num)) return "Must be a valid number";
    if (num < 0) return "Cannot be negative";
    if (num > 1) return "Creation fee too high (max 1 ETH)";
    return null;
  };

  const validateHolderThresholdBps = (value: string): string | null => {
    const num = parseFloat(value);
    if (isNaN(num)) return "Must be a valid number";
    if (num < 0) return "Cannot be negative";
    if (num > 100) return "Cannot exceed 100% (10000 BPS)";
    return null;
  };
  ...
  const isLockDurationValid: boolean =
    !validationErrors.tLockDur &&
    !validationErrors.lpLockDur &&
    !validationErrors.tLckPrcnt &&
    tLockDur.trim() !== "" &&
    lpLockDur.trim() !== "" &&
    tLckPrcnt.trim() !== "";
  ...
  const isCreationFeeValid: boolean =
    ...
    !validationErrors.creationHolderThresholdBps &&
    creationFeeETH.trim() !== "" &&
    creationHolderThresholdBps.trim() !== "";
    ...
    <div className="p-4 max-w-6xl mx-auto mt-8 space-y-6">
      <div className="text-center mb-8 text-gray-800 dark:text-white">
        ...
        <h2 className="text-sm sm:text-base">
          {SAFU_LAUNCHER_ADDRESSES_V3[networkInfo.chainId]}
        </h2>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
          <div className="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
            <div className="text-2xl font-bold text-blue-600 dark:text-blue-400">
              {Number(combinedMetrics?.[9]) / 100 || "0"}%
            </div>
          ...
          <div className="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
            <div className="text-2xl font-bold text-green-600 dark:text-green-400">
              {combinedMetrics?.[13]
                ? ethers.formatEther(combinedMetrics?.[13].toString())
                : "0"}
            </div>
            <div className="text-sm text-gray-600 dark:text-gray-400">
              Initial Pool ETH
          ...
          <div className="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
            <div className="text-2xl font-bold text-purple-600 dark:text-purple-400">
              {combinedMetrics?.[12]
                ? ethers.formatEther(combinedMetrics?.[12].toString())
                : "0"}
            </div>
            <div className="text-sm text-gray-600 dark:text-gray-400">
              Dev Reward ETH
          ...
          <div className="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
            <div className="text-2xl font-bold text-orange-600 dark:text-orange-400">
              {(Number(combinedMetrics?.[10]) / Number(combinedMetrics?.[11])) *
                100}
              %
            </div>
            <div className="text-sm text-gray-600 dark:text-gray-400">
          ...
          <div className="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
            <div className="text-2xl font-bold text-blue-600 dark:text-blue-400">
              {Number(combinedMetrics?.[22]) / 1e18 || "0"} ETH
            </div>
            <div className="text-sm text-gray-600 dark:text-gray-400">
              Deployment Fee (ETH)
            </div>
          ...
          <div className="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
            <div className="text-2xl font-bold text-blue-600 dark:text-blue-400">
              {Number(combinedMetrics?.[23]) / 100 || "0"}%
            </div>
            <div className="text-sm text-gray-600 dark:text-gray-400">
              Free Deploy Threshold (%)
            </div>
          </div>
          <div className="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
            <div className="text-2xl font-bold text-blue-600 dark:text-blue-400">
              {combinedMetrics?.[14]
                ? secondsToDays(Number(combinedMetrics[14]))
                : "0"}
            </div>
            <div className="text-sm text-gray-600 dark:text-gray-400">
              Token Lock Duration (Days)
            </div>
          </div>
          <div className="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
            <div className="text-2xl font-bold text-blue-600 dark:text-blue-400">
              {combinedMetrics?.[15]
                ? secondsToDays(Number(combinedMetrics[15]))
                : "0"}
            </div>
            <div className="text-sm text-gray-600 dark:text-gray-400">
          ...
          <div className="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
            <div className="text-2xl font-bold text-blue-600 dark:text-blue-400">
              {Number(combinedMetrics?.[16]) / 100 || "0"}%
            </div>
            <div className="text-sm text-gray-600 dark:text-gray-400">
              Token Lock Percent (%)
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="space-y-4">
            <div className="bg-yellow-50 dark:bg-yellow-900/20 p-4 rounded-lg border border-yellow-200 dark:border-yellow-800">
              ...
              <p className="text-yellow-800 dark:text-yellow-200 text-sm mb-3">
                Reserved ETH:{" "}
                {combinedMetrics?.[7]
                  ? ethers.formatEther(combinedMetrics?.[7].toString())
                  : "0"}{" "}
                ETH
              </p>
              <ActionButton
          ...
          <div className="space-y-4">
            <div className="bg-yellow-50 dark:bg-yellow-900/20 p-4 rounded-lg border border-yellow-200 dark:border-yellow-800">
              <p className="text-yellow-800 dark:text-yellow-200 text-sm mb-2">
                ⚠️ <strong>Warning:</strong> This can only recover tokens that
                were NOT created by SafuLauncher. Platform-created tokens cannot
                be recovered.
              </p>
            </div>
          ...
          <div className="space-y-4">
            <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg border border-blue-200 dark:border-blue-800">
              <p className="text-blue-800 dark:text-blue-200 text-sm">
                <strong>Info:</strong> This address receives all platform fees
                including creation fees, trade fees, and listing fees.
              </p>
            </div>
        ...
        >
          <div className="space-y-4">
            <InputField
              ...
              placeholder="0.3 (0.3%)"
              currentValue={Number(combinedMetrics?.[9]) / 100}
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
              <InputField
                ...
                placeholder="5 (5%)"
                currentValue={Number(combinedMetrics?.[17]) / 100}
            ...
            <div className="grid grid-cols-3 gap-3">
              <InputField
                ...
                placeholder="25"
                currentValue={Number(combinedMetrics?.[20])}
              ...
              <InputField
                ...
                placeholder="70"
                currentValue={Number(combinedMetrics?.[19])}
              ...
              <InputField
                ...
                placeholder="2 (2%)"
                currentValue={Number(combinedMetrics?.[21]) / 100}
        ...
        <FormSection
          title="🔒 Lock Duration Configuration"
          tooltip="Configure token and LP lock durations and percentages"
        >
          <div className="space-y-4">
            <div className="grid grid-cols-3 gap-3">
              <InputField
                ...
                tooltip="How long tokens are locked after listing (minimum 7 days)"
                validator={validateLockDurationDays}
                fieldName="tLockDur"
                unit="days"
                validationErrors={validationErrors}
                handleValidation={handleValidation}
              ...
              <InputField
                ...
                tooltip="How long LP tokens are locked after listing (minimum 7 days)"
                validator={validateLockDurationDays}
                fieldName="lpLockDur"
                unit="days"
                validationErrors={validationErrors}
              ...
              />
              <InputField
                label="Token Lock Percentage (%)"
                value={tLckPrcnt}
                onChange={setTLckPrcnt}
                placeholder="5"
                type="number"
                currentValue={Number(combinedMetrics?.[16]) / 100 || 0}
                tooltip="Percentage of tokens to be locked"
                validator={validateLockPercentage}
                fieldName="tLckPrcnt"
                unit="%"
                validationErrors={validationErrors}
                handleValidation={handleValidation}
            ...
            <ActionButton
              onClick={() =>
                writeContract({
                  ...
                  args: [
                    BigInt(daysToSeconds(parseInt(tLockDur))), // convert days to seconds
                    BigInt(daysToSeconds(parseInt(lpLockDur))), // convert days to seconds
                    BigInt(parseFloat(tLckPrcnt) * 100), // convert percentage to basis points
                  ],
                })
              }
              isValid={isLockDurationValid}
            >
        ...
        <FormSection
          ...
          tooltip="Configure fees for token creation and holder thresholds"
        >
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
              <InputField
                label="Deployment Fee (ETH)"
                value={creationFeeETH}
                onChange={setCreationFeeETH}
                placeholder="0.001"
                type="number"
                currentValue={Number(combinedMetrics?.[22]) / 1e18 || 0}
                tooltip="Fee charged to non-holders for token creation"
                validator={validateCreationFeeETH}
                fieldName="creationFeeETH"
                unit="ETH"
                validationErrors={validationErrors}
                handleValidation={handleValidation}
              />
              <InputField
                label="Free Deployment Threshold (%)"
                value={creationHolderThresholdBps}
                onChange={setCreationHolderThresholdBps}
                placeholder="0.1"
                type="number"
                currentValue={Number(combinedMetrics?.[23]) / 100 || 0}
                tooltip="SAFU token holding percentage for free creation"
                validator={validateHolderThresholdBps}
                fieldName="creationHolderThresholdBps"
            ...
            <div className="bg-blue-50 dark:bg-blue-900/20 p-3 rounded-lg">
              <p className="text-blue-700 dark:text-blue-300 text-sm">
                <strong>Note:</strong> Users holding ≥
                {creationHolderThresholdBps || "0"}% of SAFU total supply can
                create tokens for free. Others pay {creationFeeETH || "0"} ETH.
              </p>
            </div>
            <ActionButton
              onClick={() =>
                writeContract({
                  ...
                  args: [
                    ethers.parseEther(creationFeeETH || "0"), // convert ETH to wei
                    BigInt(parseFloat(creationHolderThresholdBps) * 100), // convert percentage to basis points
                  ],
                })
              }
              isValid={isCreationFeeValid}
            >
      ...
      <div className="bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
          ...
          >
            <div
              className={`w-2 h-2 rounded-full ${
                isRecoverERC20Valid ? "bg-green-500" : "bg-gray-300"
              }`}
            ></div>
          ...
          >
            <div
              className={`w-2 h-2 rounded-full ${
                isRevenueRecipientValid ? "bg-green-500" : "bg-gray-300"
              }`}
            ></div>
...
export default AdminContractConfigV3;
