...
export default function Launch(): JSX.Element {
  ...
  const validateForm = useCallback((): ValidationError[] => {
    ...
    if (enableWhitelist) {
      ...
      whitelistUpload.forEach((addr, index) => {
        ...
        if (addr.cap <= 0 || addr.cap > maxWhitelist) {
          errors.push({
            field: "whitelist",
            message: `Entry ${index + 1}: max cap for whitelisted addrs is ${maxWhitelist}%.`,
          });
        }
      });
    ...
    if (enableBundle) {
      ...
      const capPercent = isMaxWalletAmountOnSafu ? maxWalletAmountOnSafu : maxWhitelist; // default 2% when custom cap is off
