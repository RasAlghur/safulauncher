 async verifyTransaction(tokenAddress: string, identifier: string) {
    try {
      const findtoken = await this.tokensService.findOne({
        identifier: identifier,
      });

      if (!findtoken) return;

      const etherscanApiKey = this.config.getOrThrow('ETHERSCAN_API_KEY');
      ...
      const verificationData: Record<string, string> = {
        ...
        sourceCode: JSON.stringify(this.inputSourceCode()), // Your contract source code
        ...
        constructorArguments: findtoken.data.verifyparameter, // Encoded constructor arguments
      };

      console.log('tokenAddress', tokenAddress);
      console.log('verificationData', verificationData);

      const etherscanApiUrl = this.config.getOrThrow('ETHERSCAN_API_URL');
      const { data } = await firstValueFrom(
        this.httpService
          .post(etherscanApiUrl, new URLSearchParams(verificationData), {
      ...
      const statusUrl = `${etherscanApiUrl}&module=contract&action=checkverifystatus&apikey=${etherscanApiKey}&guid=${data.result}`;
    ...
    try {
      ...
      console.error(
        '❌ [startup] Fatal error reading or parsing Solidity:',
        err,
      );
      process.exit(1);
    }
