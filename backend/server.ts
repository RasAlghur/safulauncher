...
app.get('/api/ohlc/:tokenAddress', (req, res) => {
  ...
  try {
    ...
    const lastTxTime = new Date(relevant[relevant.length - 1].timestamp).getTime();
    const totalTimeSpan = lastTxTime - firstTxTime;

    console.log(`Time range: ${new Date(firstTxTime).toISOString()} to ${new Date(lastTxTime).toISOString()}`);
    console.log(`Total time span: ${Math.floor(totalTimeSpan / (24 * 60 * 60 * 1000))} days`);
    ...
    for (const tx of relevant) {
      ...
      const bucketKey = Math.floor(timestamp / config.bucketSize) * config.bucketSize;
    ...
    if (shouldLimit) {
      console.log(`Limited bars from ${bars.length} to ${finalBars.length} (keeping most recent)`);
    }
    ...
    console.log(`Generated ${filledBars.length} OHLC bars covering full transaction history`);
...
function shouldFillGaps(resolution: string): boolean {
  ...
  return ['1h', '2h', '4h', '8h', '12h', '1D', '3D', '1W', '1M'].includes(resolution);
}
...
app.get('/api/timeframes/:tokenAddress', (req: Request, res: Response) => {
  ...
  try {
    ...
    // Sort by timestamp
    tokenTxs.sort((a, b) => new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime());
    ...
    if (dataSpan >= 86400000) { // At least 1 day
...
app.get('/api/stats/:tokenAddress', (req: Request, res: Response) => {
  ...
  try {
    ...
    const prices = relevant.map(tx => parseFloat(tx.ethAmount) / parseFloat(tx.tokenAmount));
    const volume = relevant.reduce((sum, tx) => sum + parseFloat(tx.ethAmount), 0);

    res.json({
      volume,
      trades: relevant.length,
      priceChange: prices.length > 1 ? ((prices[prices.length - 1] - prices[0]) / prices[0] * 100) : 0,
...
app.get('/api/debug/transactions/:tokenAddress', (req: Request, res: Response) => {
  const { tokenAddress } = req.params;
  ...
  try {
    ...
    res.json({
      ...
      summary: {
        totalVolume: relevant.reduce((sum, tx) => sum + parseFloat(tx.ethAmount), 0),
        avgPrice: relevant.length > 0
          ? relevant.reduce((sum, tx) => sum + (parseFloat(tx.ethAmount) / parseFloat(tx.tokenAmount)), 0) / relevant.length
          : 0
      }
    });
  } catch (error) {
